...
class Dependency {
	...
	get loc() {
		...
		const loc = {};
		if (this.locSL > 0) {
			loc.start = { line: this.locSL, column: this.locSC };
		}
		if (this.locEL > 0) {
			loc.end = { line: this.locEL, column: this.locEC };
		}
		if (this.locN !== undefined) {
			loc.name = this.locN;
		}
		if (this.locI !== undefined) {
			loc.index = this.locI;
		}
		return loc;
	}
	...
	set loc(loc) {
		if ("start" in loc && typeof loc.start === "object") {
			this.locSL = loc.start.line || 0;
			this.locSC = loc.start.column || 0;
		} else {
			this.locSL = 0;
			this.locSC = 0;
		}
		if ("end" in loc && typeof loc.end === "object") {
			this.locEL = loc.end.line || 0;
			this.locEC = loc.end.column || 0;
		} else {
			this.locEL = 0;
			this.locEC = 0;
		}
